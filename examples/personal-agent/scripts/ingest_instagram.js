...
async function ingestInstagramMessages() {
  ...
  const htmlFiles = globSync(
    "instagram-*/your_instagram_activity/messages/inbox/**/*.html"
  );
    ...
    $(".pam._3-95._2ph-._a6-g.uiBoxWhite.noborder").each((_, el) => {
      ...
      allEvents.push({
        ...
        metadata: {
          ...
          timestamp: isoDate,
          direction:
            sender.toLowerCase() === "burak emre" ||
            sender.toLowerCase().includes("burak")
              ? "outbound"
              : "inbound",
          sender: sender,
        },
  ...
  console.log(
    `Parsed ${allEvents.length} Instagram messages. Starting chunked upload...`
  );
  ...
  for (let i = 0; i < allEvents.length; i += CHUNK_SIZE) {
    ...
    console.log(
      `Pushing Instagram chunk ${Math.floor(i / CHUNK_SIZE) + 1} / ${Math.ceil(allEvents.length / CHUNK_SIZE)}`
    );
