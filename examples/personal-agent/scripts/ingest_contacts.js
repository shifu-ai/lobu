...
async function ingestContacts() {
  ...
  const vcfData = readFileSync(
    "Takeout 2/Contacts/All Contacts/All Contacts.vcf",
    "utf-8"
  );
  ...
  for (let i = 0; i < people.length; i += CHUNK_SIZE) {
    ...
    console.log(
      `Pushing contacts chunk ${Math.floor(i / CHUNK_SIZE) + 1} / ${Math.ceil(people.length / CHUNK_SIZE)}`
    );
    try {
