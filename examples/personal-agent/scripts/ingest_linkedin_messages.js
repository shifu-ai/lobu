...
async function ingestLinkedInMessages() {
  ...
  const csv = readFileSync("Basic_LinkedInDataExport_07-05-2026.zip/messages.csv", "utf-8");
  ...
  const events = records
    ...
    .map((r, idx) => {
      let isoDate = "";
      try { isoDate = new Date(r["DATE"]).toISOString(); } 
      catch (e) { isoDate = r["DATE"]; }
  ...
  for (let i = 0; i < events.length; i += CHUNK_SIZE) {
    ...
    console.log(`Pushing chunk ${Math.floor(i/CHUNK_SIZE) + 1} / ${Math.ceil(events.length/CHUNK_SIZE)}`);
    try {
