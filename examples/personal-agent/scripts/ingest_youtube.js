...
async function ingestYouTube() {
  ...
  const historyFile =
    "Takeout 3/YouTube and YouTube Music/history/watch-history.html";
  ...
  console.log(
    `Parsed ${knowledgeEvents.length} YouTube watch events. Starting chunked upload...`
  );
  ...
  for (let i = 0; i < knowledgeEvents.length; i += CHUNK_SIZE) {
    ...
    console.log(
      `Pushing YouTube chunk ${Math.floor(i / CHUNK_SIZE) + 1} / ${Math.ceil(knowledgeEvents.length / CHUNK_SIZE)}`
    );
    try {
