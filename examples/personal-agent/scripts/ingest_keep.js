...
async function ingestKeep() {
  ...
  for (const file of jsonFiles) {
    try {
      ...
      if (data.listContent) {
        content += data.listContent.map(item => "- " + item.text + (item.isChecked ? " (done)" : "")).join("\\n");
      }
      ...
      const metadata = {
        ...
        timestamp: data.createdTimestampUsec ? new Date(data.createdTimestampUsec / 1000).toISOString() : null,
  ...
  console.log(`Parsed ${knowledgeEvents.length} Keep notes. Starting chunked upload...`);
  ...
  for (let i = 0; i < knowledgeEvents.length; i += CHUNK_SIZE) {
    ...
    console.log(`Pushing notes chunk ${Math.floor(i/CHUNK_SIZE) + 1} / ${Math.ceil(knowledgeEvents.length/CHUNK_SIZE)}`);
    try {
