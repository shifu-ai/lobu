 ...
 * per-action `execute` handlers, instead of hand-writing a class that switches
 * on `ctx.feedKey` / `ctx.actionKey`. The feed/action keys are taken from the
 * record keys, so they're never repeated.
 *
 ...
 * `sync`/`execute` functions) is captured by the returned class.
 *
 * @example
 * ```ts
...
function buildDefinition(spec: ConnectorSpec): ConnectorDefinition {
	...
	if (spec.feeds) {
		definition.feeds = Object.fromEntries(
			Object.entries(spec.feeds).map(
				([key, feed]): [string, FeedDefinition] => [
					key,
					{
						key,
						name: feed.name,
