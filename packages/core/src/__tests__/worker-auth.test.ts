...
describe("worker auth token", () => {
  ...
  test.each([
    ...
    const token = encrypt(JSON.stringify({ userId: "u", conversationId: "c", channelId: "ch", deploymentName: "d", timestamp: Date.now(), tokenKind: "run", runId: 1, agentId: "a", courseToolScope: { ownerUserId: "u", agentId: "a", courseEntityId: "course:u:a" }, ...override }));
    expect(verifyWorkerToken(token)).toBeNull();
  });
