...
export const ManageWatchersSchema = Type.Object({
  action: Type.Union(
    [
      Type.Literal('create', { description: 'Create a watcher with prompt + sources.' }),
      Type.Literal('update', { description: 'Patch watcher config.' }),
      Type.Literal('create_version', { description: 'Create a new versioned watcher config.' }),
      Type.Literal('complete_window', { description: 'Submit a watcher window result.' }),
      Type.Literal('trigger', { description: 'Manually fire a watcher run.' }),
      Type.Literal('delete', { description: 'Bulk-delete watchers.' }),
      Type.Literal('set_reaction_script', { description: 'Attach/remove a TypeScript reaction script.' }),
      Type.Literal('get_versions', { description: 'List a watcher\u2019s version history.' }),
      Type.Literal('get_version_details', { description: 'Fetch full version config.' }),
      Type.Literal('get_component_reference', { description: 'Static component/data-type documentation.' }),
      Type.Literal('submit_feedback', { description: 'Submit per-field corrections on a window.' }),
      Type.Literal('get_feedback', { description: 'Retrieve feedback for a watcher.' }),
      Type.Literal('list_promoted', { description: 'List entities promoted by a watcher.' }),
      Type.Literal('create_from_version', { description: 'Spin up watchers per entity from a template version.' }),
    ],
