...
function withContent<T extends UnifiedSearchResult>(
	result: T,
	content: ContentSnippet[],
): T {
...
async function fetchContentSnippets(
	...
	return result.content.map((c) => ({
		...
		title: c.title,
		text_content:
			c.payload_text.length > 500
				? c.payload_text.slice(0, 500) + "..."
				: c.payload_text,
...
export async function search(
	...
	if (!args.query && !args.entity_id && !args.query_embedding?.length) {
		throw new ToolUserError(
			"Must provide either query, entity_id, or query_embedding",
			400,
		);
	}
	...
	if (args.entity_id) {
		...
		if (entity) {
			return withContent(
				await formatEntityResult([entity], args, ctx),
				contentSnippets,
			);
		}
		return withContent(
	...
	if (results.length > 0) {
		return withContent(
			await formatEntityResult(results, args, ctx),
			contentSnippets,
		);
	}
...
function validateCourseEntityIds(value: unknown): string[] | undefined {
	...
	if (!Array.isArray(value) || value.length > 20) {
		throw new ToolUserError(
			"entity_ids must be an array of at most 20 course ids",
			400,
		);
	}
	...
	if (ids.some((item) => !COURSE_ENTITY_ID_PATTERN.test(item))) {
		throw new ToolUserError(
			"entity_ids contains an invalid course entity id",
			400,
		);
	}
	return [...new Set(ids)];
...
async function fetchTopEntitiesByType(
	...
	Array<{ entity_type: string; entities: Array<{ id: number; name: string }> }>
> {
	const sql = getDb();
...
async function queryEntities(
	...
	const embeddingParamIdx = hasEmbedding
		? addParam(toVectorLiteral(args.query_embedding!))
		: null;
	...
	if (query) {
		if (fuzzyEnabled) {
			...
			conditions.push(
				hasEmbedding
					? `(${textCondition} OR e.embedding IS NOT NULL)`
					: textCondition,
			);
		} else {
	...
	if (args.entity_type)
		conditions.push(`et.slug = $${addParam(args.entity_type)}`);
	if (args.parent_id)
		conditions.push(`e.parent_id = $${addParam(args.parent_id)}`);
	if (args.category)
		conditions.push(
			`e.metadata::jsonb->>'category' = $${addParam(args.category)}`,
		);
	if (args.market) {
	...
	if (args.metadata_filter) {
		for (const [key, value] of Object.entries(args.metadata_filter)) {
			conditions.push(
				`e.metadata->>'${key.replace(/'/g, "''")}' = $${addParam(value)}`,
			);
		}
	}
	...
	if (hasEmbedding) {
		...
		const nameSimExpr =
			queryParamIdx !== null
				? `similarity(LOWER(e.name), LOWER($${queryParamIdx}))`
				: "0";
...
async function formatEntityResult(
	...
	if (matches.length === 1) {
		...
		const pausedConnections =
			connections?.filter((c) => c.status === "paused").length || 0;
