...
function makeSql(agents: FakeAgentRow[], pauseCalls: string[]): SqlLike {
	return (async (strings: TemplateStringsArray, ...values: unknown[]) => {
		...
		const match = agents.find(
			(a) => a.id === rawAgentId && a.organization_id === organizationId,
		);
...
describe("handleWakeAgentTask — trusted course fire gate", () => {
	const trustedWake = {
		...
		trustedCourseScope: {
			...
			courseKey: "a",
			courseDisplayName: "A 課",
			resolutionSource: "toolbox_calendar_course_resolver" as const,
			resolutionMatchedBy: ["instructor_alias"] as const,
			scopeVersion: 1 as const,
		},
		taskKind: "opp_coach_rehearsal_prompt" as const,
		delivery: "line" as const,
		triggerSource: "google_calendar" as const,
		calendarEventRef: {
			accountRef: "acct",
			eventId: "event",
			eventVersion: "v1",
			eventTitle: "課程 A",
			eventStartAt: "2026-08-01T00:00:00Z",
		},
		scheduledFor: "2026-07-31T00:00:00Z",
	};
	const resolved = {
		trust: {
			ownerUserId: "user-1",
		...
		course: { courseKey: "a", courseEntityId: "course:a", displayName: "A 課" },
		resolution: {
			confidence: "high" as const,
			matchedBy: ["explicit_course_key"] as ["explicit_course_key"],
		},
		context: {
			contextPackId: "pack",
			contextVersion: 1,
		...
		await handleWakeAgentTask(
			{
				...
				queueProducer: { enqueueMessage } as never,
				resolveScheduledCourseContext,
			},
			{
				__organization_id: ORG,
				...
				__scheduled_job_external_key:
					"google_calendar:acct:event:opp_coach_rehearsal_prompt",
				__scheduled_job_tick: "2026-07-31T00:00:00Z",
				__scheduled_task_run_id: 42,
				agent_id: BARE_AGENT_ID,
				prompt: "coach",
				thread_id: "existing",
				reason: "trusted-course-calendar-wake",
				trustedCourseWake: trustedWake,
			},
		);
		...
		await handleWakeAgentTask(
			{
				sql: (async (strings: TemplateStringsArray) =>
					strings.raw.join("").includes("SELECT id FROM agents")
						? [{ id: BARE_AGENT_ID }]
						: []) as never,
				sessionManager: {} as never,
				queueProducer: { enqueueMessage } as never,
				resolveScheduledCourseContext: resolver,
			},
			{
				__organization_id: ORG,
				__created_by_user: "user-1",
	...
	test("rejects invalid trusted provenance before resolving, creating, or touching a conversation", async () => {
		...
		await handleWakeAgentTask(
			{
				sql: (async (strings: TemplateStringsArray) =>
					strings.raw.join("").includes("SELECT id FROM agents")
						? [{ id: BARE_AGENT_ID }]
						: []) as never,
				sessionManager: { getSession, touchSession, setSession } as never,
				queueProducer: { enqueueMessage } as never,
				resolveScheduledCourseContext: mock(async () => resolved),
			},
			{
				__organization_id: ORG,
				__created_by_user: "user-1",
	...
	test("uses one durable queue identity from scheduled job and task run across handler retries", async () => {
		const enqueueMessage = mock(async () => "worker-run");
		const deps = {
			sql: (async (strings: TemplateStringsArray) =>
				strings.raw.join("").includes("owner_platform")
					? [{ id: BARE_AGENT_ID }]
					: [{ id: BARE_AGENT_ID }]) as never,
			sessionManager: {
				getSession: mock(async () => ({
					conversationId: "existing",
					channelId: "api_user-1",
		...
		await handleWakeAgentTask(deps, payload);
		await handleWakeAgentTask(deps, payload);
		expect(enqueueMessage).toHaveBeenCalledTimes(2);
		expect(enqueueMessage.mock.calls.map((call) => call[0].messageId)).toEqual([
			"scheduled-job-1-run-42",
			"scheduled-job-1-run-42",
		]);
		expect(enqueueMessage.mock.calls.map((call) => call[1])).toEqual([
			{ singletonKey: "scheduled-job-1-run-42", durableSingleton: true },
			{ singletonKey: "scheduled-job-1-run-42", durableSingleton: true },
		]);
	});
