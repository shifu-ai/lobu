...
import { registerScheduledJobsTicker, resolveWakeAgentId } from './scheduled-jobs-service';
...
async function resolveScheduledCourseContextAtFire(input:{payload:MessagePayload;trustedWake:TrustedCourseWakeV1}):Promise<ResolvedCourseExecutionContext|null>{
  const result=await attachCourseContextForReviewedScope(input.payload,{baseUrl:process.env.TOOLBOX_COURSE_CONTEXT_URL?.trim()??'',secret:process.env.TOOLBOX_INTERNAL_SECRET?.trim()??'',propagateInfrastructureErrors:true});
...
export async function handleWakeAgentTask(
  ...
  if(hasTrustedWake){
    ...
    if(!fire){logger.warn({category:'trusted_course_fire_gate',scheduledJobId:p.__scheduled_job_id,scheduledTaskRunId:p.__scheduled_task_run_id},'[task] trusted course wake rejected deterministically');return;}
