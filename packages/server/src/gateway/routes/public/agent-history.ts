...
async function readSessionMessages(
  ...
  // The disk file may be empty (a fresh pod has no workspaces/ tree on
  // a multi-replica gateway). Try the PG snapshot first; fall through
  // to the disk read if the snapshot is missing so local-dev
  // workspaces/* trees keep working without DB migrations.
...
async function readSessionStats(
  ...
  // Same fallback shape as readSessionMessages — DB first, disk read if
  // absent.
  let content: string | null = await readLatestSnapshotJsonl(
    agentId,
    organizationId
