...
export class CoreServices {
	...
	private async initializeSessionServices(): Promise<void> {
		...
		// Cross-replica SSE delivery: broadcasts on one pod reach clients whose
		// SSE connection is pinned to another pod (see SseFanout). Starts the
		// shared-listener subscription and wires the publisher; fails open to
		// local-only if LISTEN is unavailable.
