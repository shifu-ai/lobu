...
function parseBundle(v: unknown): ValidBundle { const x=obj(v); profile(x.profile); const c=obj(x.context); const evidence=obj(x.evidence); const version=c.version; if(!Number.isInteger(version)||(version as number)<=0||typeof c.stale!=='boolean'||(c.confidence!=='high'&&c.confidence!=='medium'&&c.confidence!=='low')||!Array.isArray(evidence.confirmed)||!Array.isArray(evidence.candidates)||evidence.confirmed.length>100||evidence.candidates.length>100)throw new ToolboxCourseContextResponseError(); evidence.confirmed.map(evidenceRef); evidence.candidates.map(evidenceRef); const result={course:canonicalCourse(x.course),context:{agentMd:str(c.agentMd,MAX_SUMMARY),contextPackId:str(c.contextPackId),version:version as number,confidence:c.confidence,generatedAt:str(c.generatedAt),lastIndexedAt:nullable(c.lastIndexedAt),stale:c.stale}}; if(JSON.stringify(x).length>MAX_TOTAL)throw new ToolboxCourseContextResponseError(); return result; }
