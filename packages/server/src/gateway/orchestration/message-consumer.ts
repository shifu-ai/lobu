...
export class MessageConsumer {
  ...
  private async emitExecutionSelection(data:MessagePayload):Promise<void>{const mode=data.trustedExecutionScope?.mode==='course'?'course':data.trustedExecutionScope?.mode==='onboarding'?'onboarding':'personal';const skill=data.trustedExecutionScope?.mode==='course'&&data.trustedExecutionScope.activeSpecializedSkill==='opp-coach'?'opp-coach':'none';const base={trace_id:extractTraceId(data)??`tr_${createHash('sha256').update(data.messageId??data.conversationId).digest('hex').slice(0,32)}`,journey_id:'course_context_gate',service:'lobu' as const,module:'message-consumer',status:'ok'};await Promise.all([this.emitJourneyFailOpen({...base,event:'context.scope.selected',mode}),this.emitJourneyFailOpen({...base,event:'context.skill.selected',skill})]);}
