...
function consumeTrustedScheduledTaskKind(data:MessagePayload):'sales_rehearsal'|undefined{const execution=data.trustedScheduledExecution;delete data.trustedScheduledExecution;if(!execution||execution.source!=='internal_scheduler'||execution.taskKind!=='sales_rehearsal'||execution.ownerUserId!==data.userId||execution.agentId!==data.agentId||execution.conversationId!==data.conversationId)return undefined;return execution.taskKind;}
...
export class MessageConsumer {
  ...
  private async dispatchCourseContextBoundary(data: MessagePayload, deploymentName: string): Promise<boolean> {
    const trustedScheduledTaskKind=consumeTrustedScheduledTaskKind(data);
    ...
    try { if (this.courseContextResolver === attachCourseContextForReviewedScope) {
      ...
      const activeCourseSkill=selectActiveCourseSkill({available:courseSkillContext,message:typeof shadowPayload.messageText==='string'?shadowPayload.messageText:'',trustedScheduledTaskKind});
