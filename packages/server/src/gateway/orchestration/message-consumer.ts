...
export class MessageConsumer {
	...
	private readonly courseContextResolver: (
		payload: MessagePayload,
	) => Promise<CourseContextGateResult | void>;
	private readonly courseContextRollout: ReturnType<
		typeof parseCourseContextRolloutConfig
	>;
	private readonly journeyEmitter: (
		event: Parameters<typeof emitJourneyObsEvent>[0],
		signal?: AbortSignal,
	) => Promise<void>;
	private async emitJourneyFailOpen(
		event: Parameters<typeof emitJourneyObsEvent>[0],
	): Promise<void> {
		const controller = new AbortController();
		let timeout: ReturnType<typeof setTimeout> | undefined;
		try {
			const operation = this.journeyEmitter(event, controller.signal);
	...
	private async dispatchCourseContextBoundary(
		data: MessagePayload,
		deploymentName: string,
	): Promise<boolean> {
		const gateMode: CourseContextGateMode = data.scheduledCourseContext
			? "enforce"
			: this.courseContextRollout.mode;
		if (gateMode === "off" || gateMode === "shadow")
		...
		const isNonBindingEvaluation =
			gateMode === "shadow" || gateMode === "single_course";
		const shadowPayload = isNonBindingEvaluation
			? {
					...data,
					platformMetadata: { ...data.platformMetadata },
					resolvedCourseContext: undefined,
				}
			: data;
		try {
			if (this.courseContextResolver === attachCourseContextForReviewedScope) {
				...
				if (!personalBypass)
					try {
						settings =
							data.agentId && this.agentSettingsStore
								? await this.agentSettingsStore.getSettings(data.agentId)
								: null;
					} catch {
						logger.warn(
							{ category: "course_skill_settings", agentId: data.agentId },
							"Course skill settings unavailable; using deterministic message scope",
						);
					}
				const courseSkillContext = resolveCourseSkillContextMetadata(
				...
				result = await attachCourseContextForReviewedScope(shadowPayload, {
					...
					sessionManager: isNonBindingEvaluation
						? undefined
						: this.sessionManager,
					sessionKey: computeSessionKey(data),
					courseSkillEnabled: courseSkillContext.enabled,
					courseSkillContextFields: courseSkillContext.contextFields,
					courseSkillRetrievalTerms: courseSkillContext.retrievalTerms,
					courseSkillRetrievalLimit: courseSkillContext.retrievalLimit,
			...
			if (gateMode !== "shadow") throw error;
			result = { status: "not_required" };
			await this.emitJourneyFailOpen({
				trace_id:
					extractTraceId(data) ??
					`tr_${createHash("sha256")
						.update(data.messageId ?? data.conversationId)
						.digest("hex")
						.slice(0, 32)}`,
				journey_id: "course_context_gate",
				event: "context.course.missing",
				service: "lobu",
		...
		if (gateMode === "shadow") {
			const resolved =
				result?.status === "ready"
					...
					: result?.status === "context_unavailable"
						? result.resolvedCourse
						: undefined;
			const legacy = await readLegacyCourseContext(data);
			const comparison = compareCourseContextIdentity(resolved, legacy);
			await this.emitJourneyFailOpen({
				trace_id:
					extractTraceId(data) ??
					`tr_${createHash("sha256")
		...
		if (
			gateMode === "enforce" &&
			this.courseContextRollout.legacyFallback &&
			result?.status === "context_unavailable" &&
			result.resolvedCourse
		) {
			const legacy = await readLegacyCourseContext(data);
			if (
				compareCourseContextIdentity(result.resolvedCourse, legacy) === "match"
			)
				await this.emitJourneyFailOpen({
					trace_id:
						extractTraceId(data) ??
		...
		if (
			result?.status === "ready" &&
			!hasTrustedCourseContext(data, result.context)
		) {
			delete data.resolvedCourseContext;
			result = {
		...
		if (gateMode === "single_course") {
			const isSingleCourseDefault =
				result?.status === "ready" &&
				result.context.resolution.matchedBy.includes("single_course_default");
			if (isSingleCourseDefault)
				data.resolvedCourseContext = shadowPayload.resolvedCourseContext;
			...
			// worker without canonical context.
				...
				result = {
					status: "context_unavailable",
					reasonCode: "single_course_not_confirmed",
				};
		}
		if (
			data.resolvedCourseContext &&
			!hasTrustedCourseContext(data, data.resolvedCourseContext)
		) {
			delete data.resolvedCourseContext;
			result = {
				status: "context_unavailable",
		...
		data.runJobToken = mintRunJobToken(
			...
			data.conversationId,
			deploymentName,
			gateMode === "enforce" && result?.status === "ready",
		);
		await armTurnTimeout(this.queue, {
			messageId: data.messageId,
			channelId: data.channelId,
		...
		if (result?.status === "ready" && result.replay && this.sessionManager) {
			let marked =
				await this.sessionManager.markPendingCourseSelectionDispatched(
					computeSessionKey(data),
					result.replay.pendingId,
					data.userId,
					data.agentId,
					result.replay.messageId,
				);
			for (
				let attempt = 1;
				marked.status === "failed" && attempt < 3;
				attempt++
			)
				marked = await this.sessionManager.markPendingCourseSelectionDispatched(
					computeSessionKey(data),
					result.replay.pendingId,
					data.userId,
					data.agentId,
			...
			if (marked.status === "failed")
				logger.warn(
					{
						category: "pending_dispatch_mark",
						pendingId: result.replay.pendingId,
					},
					"Course selection dispatched; dispatch marker deferred",
				);
			const cleared = await this.sessionManager.clearPendingCourseSelection(
				computeSessionKey(data),
				result.replay.pendingId,
				data.userId,
				data.agentId,
			...
			if (cleared.status !== "cleared" && cleared.status !== "stale")
				logger.warn(
					{ category: "pending_cleanup", pendingId: result.replay.pendingId },
					"Course selection dispatched; pending cleanup deferred",
				);
		}
		return true;
	}

	private async deliverCourseContextTerminal(
		data: MessagePayload,
		result: Exclude<
			CourseContextGateResult,
			| { status: "ready" }
		...
		const finalText =
			result.status === "clarification_required"
				...
				: result.status === "onboarding_required"
					? "目前還沒有可用的課程資料，請先完成課程設定後再試。"
					: result.status === "context_unavailable" && result.displayName
						? `目前無法取得「${clean(result.displayName)}」的課程資料，請稍後再試。`
						: "目前無法取得課程資料，請稍後再試。";
		...
		await this.queue.send(
			"thread_response",
			{
				...
				timestamp: Date.now(),
				teamId:
					data.teamId ?? getStringField(data.platformMetadata, "teamId") ?? "",
			},
			{
				...TERMINAL_DELIVERY_SEND_OPTS,
