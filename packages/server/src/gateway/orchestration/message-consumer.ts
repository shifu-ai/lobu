...
export class MessageConsumer {
	...
	) => Promise<CourseContextGateResult | undefined>;
	...
	constructor(
		...
		courseContextResolver: (
			...
			CourseContextGateResult | undefined
		> = attachCourseContextForReviewedScope,
	) {
		this.config = config;
	...
	private async dispatchCourseContextBoundary(
		...
		let result: CourseContextGateResult | undefined;
	...
	private async deliverCourseContextTerminal(
		...
		if (result.status === "clarification_required") {
			finalText = `請選擇這次要處理的課程：\n${result.candidates.map((candidate, index) => `${index + 1}. ${candidate.displayName.replace(/[\r\n\u2028\u2029]/gu, " ")}`).join("\n")}`;
		} else if (result.status === "onboarding_required") {
			finalText = "目前還沒有可用的課程資料，請先完成課程設定後再試。";
		} else {
			finalText = result.displayName
				? `目前無法取得「${result.displayName.replace(/[\r\n\u2028\u2029]/gu, " ")}」的課程資料，請稍後再試。`
				: "目前無法取得課程資料，請稍後再試。";
		}
