...
const OPPORTUNITY_COACH_MEMORY_TERMS = [
	"受眾",
	"夢想結果",
	"課程承諾",
	"Key Learning",
...
export async function attachCourseContextForReviewedScope(
	...
	const pendingExpired = Boolean(
		session?.pendingCourseSelection?.status === "pending" &&
			Date.now() - session.pendingCourseSelection.createdAt > 10 * 60_000,
	);
	if (
		pendingExpired &&
		...
		(
			await options.sessionManager.clearPendingCourseSelection(
				options.sessionKey,
				session!.pendingCourseSelection!.pendingId,
			)
		).status !== "cleared"
	)
		return {
	...
	try {
		resolution = await client.resolve({
			...
			boundCourseKey: session?.shifuCourseContext?.courseKey,
			explicitCourseKey: choice?.courseKey,
		});
	} catch {
		logger.warn(
			{ category: "resolver" },
			...
			? await retrieveCourseMemory(
					{
						...
						skillTerms: options.courseSkillEnabled
							? OPPORTUNITY_COACH_MEMORY_TERMS
							: [],
					},
	...
	> = {
		...
		context: {
			...
			stale: context.stale,
			confirmedSummary: context.agentMd.slice(0, 8000),
		},
		retrieval,
	};
