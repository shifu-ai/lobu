...
function mergeReadiness(canonical:Partial<Record<CourseReadinessField,string|null>>,retrieval:Awaited<ReturnType<typeof retrieveCourseMemory>>){const values:Partial<Record<CourseReadinessField,string>>={};const conflictedFields:CourseReadinessField[]=[];const retrievedFields=new Set<CourseReadinessField>();for(const field of READINESS_FIELDS){const canonicalValue=canonical[field]?.trim();const retrieved=[...new Set(retrieval.snippets.map((snippet)=>snippet.readinessFields[field]?.trim()).filter((value):value is string=>Boolean(value)))];if(canonicalValue){values[field]=canonicalValue;if(retrieved.some((value)=>value!==canonicalValue))conflictedFields.push(field);}else if(retrieved.length===1)values[field]=retrieved[0];else if(retrieved.length>1)conflictedFields.push(field);if(retrieved.length>0)retrievedFields.add(field);}const assessment=gradeCourseEvidenceReadiness(values);if(conflictedFields.length){assessment.level='conflicted';assessment.answerPolicy='answer_conservatively';}return{assessment,retrievedFields:[...retrievedFields]};}
