...
export interface CourseTurnDecision { courseContextRequired:boolean; }
export function decideCourseTurn(data:MessagePayload,options:{hasActiveCourse?:boolean}={}):CourseTurnDecision {
  const message = data.messageText?.trim() ?? '';
  if (PERSONAL_REMINDER.test(message) && !COURSE_INTENT.test(message)) return {courseContextRequired:false};
  const courseContextRequired=data.platformMetadata?.courseScope==='reviewed'||COURSE_INTENT.test(message)||Boolean(options.hasActiveCourse&&/^(?:繼續|接著|然後|再來|照剛才|就這個)/u.test(message));
  return {courseContextRequired};
}
export function requiresCourseContext(data: MessagePayload, options: {hasActiveCourse?:boolean} = {}): boolean {
  return decideCourseTurn(data,{hasActiveCourse:options.hasActiveCourse}).courseContextRequired;
}
...
export async function attachCourseContextForReviewedScope(data: MessagePayload, options?: CourseContextGateOptions): Promise<CourseContextGateResult> {
  ...
  if (!choice && !pending && options?.activeSpecializedSkill!=='opp-coach' && !decideCourseTurn(data, { hasActiveCourse: Boolean(session?.shifuCourseContext) }).courseContextRequired) return { status: 'not_required' };
  ...
  const specializedActive=options?.activeSpecializedSkill==='opp-coach';
  ...
  const resolvedCourseContext:NonNullable<MessagePayload['resolvedCourseContext']> = {
    ...
    context: { contextPackId: context.contextPackId, contextVersion: context.version, stale: context.stale, confirmedSummary: specializedActive&&options?.courseSkillContextFields?.length?projectRequiredCourseContext(bundle,options.courseSkillContextFields):context.agentMd.slice(0, 8000) },
    retrieval,
    ...(mergedReadiness?{readiness:mergedReadiness.assessment,evidence:[{kind:'canonical_context' as const,fields:[...new Set([...(bundle.profile.audience?['audience' as const]:[]),...(bundle.profile.coursePromise?['course_promise' as const]:[])])],sourceLabel:'已驗證的課程脈絡',sourceHash:hashIdentity(`${course.courseEntityId}:${context.contextPackId}`).slice(0,16)},...(mergedReadiness.retrievedFields.length?[{kind:'fresh_course_retrieval' as const,fields:mergedReadiness.retrievedFields,sourceLabel:'本次課程限定檢索',sourceHash:hashIdentity(retrieval.evidenceRefs.join('|')).slice(0,16)}]:[])]}:{}),
  };
