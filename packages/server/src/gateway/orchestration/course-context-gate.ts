...
export type CourseContextGateResult =
	...
	| { status: "context_unavailable"; displayName?: string; reasonCode: string };
...
export function requiresCourseContext(
	data: MessagePayload,
	options: Pick<CourseContextGateOptions, "courseSkillEnabled"> & {
		hasActiveCourse?: boolean;
	} = {},
): boolean {
	if (options.courseSkillEnabled) return true;
	if (data.platformMetadata?.courseScope === "reviewed") return true;
	const message = data.messageText?.trim() ?? "";
	if (PERSONAL_REMINDER.test(message)) return false;
...
export async function attachCourseContextForReviewedScope(
	...
	const session =
		options?.sessionManager && options.sessionKey
			? await options.sessionManager.getSession(options.sessionKey)
			: null;
	if (
	...
	if (options?.sessionKey && !options.sessionManager)
		return { status: "context_unavailable", reasonCode: "binding_unavailable" };
	const baseUrl =
	...
	data.resolvedCourseContext = {
		...
		context: {
			...
			stale: context.stale,
			confirmedSummary: context.confirmedSummary,
		},
		retrieval: { status: "skipped", eventIds: [], evidenceRefs: [] },
	};
	if (!options?.sessionManager || !options.sessionKey)
		return { status: "ready", context: data.resolvedCourseContext };
	...
	return {
		status: "ready",
		context: data.resolvedCourseContext,
