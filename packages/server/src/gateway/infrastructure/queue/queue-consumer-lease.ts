...
export function createPostgresQueueConsumerLeaseStore(
  ...
  return {
    ...
    expire: (queueName, consumerId, leaseInstanceId, now) =>
    ...
    async list(requiredQueues) {
      ...
      return rows.map((row: any) => ({
        ...
        leaseInstanceId: row.lease_instance_id,
...
export async function recordQueueConsumerHeartbeat(
  ...
  await sql`
    INSERT INTO public.queue_consumer_leases (
      queue_name, consumer_id, lease_instance_id, deployment_revision, declared_image_digest,
      started_at, last_seen_at, lease_expires_at
    ) VALUES (
      ${input.queueName}, ${input.consumerId}, ${input.leaseInstanceId}, ${input.deploymentRevision}, ${input.declaredImageDigest},
      ${input.startedAt}, ${input.now}, ${expiresAt}
    )
    ON CONFLICT (queue_name, consumer_id, lease_instance_id) DO UPDATE SET
...
export async function expireQueueConsumerLease(
  ...
  await sql`
    ...
    WHERE queue_name = ${queueName} AND consumer_id = ${consumerId} AND lease_instance_id = ${leaseInstanceId}
  `;
}
