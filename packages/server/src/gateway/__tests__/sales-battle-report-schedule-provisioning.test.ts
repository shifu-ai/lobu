...
describe("sales battle report schedule provisioning", () => {
	...
	test("builds wake_agent rows for Taipei midnight after each sales-talk weekday", () => {
		...
		expect(prompt).toContain("salesTalkDate: {{salesTalkDate}}");
		expect(prompt).toContain("Do not add extra MCP arguments.");
		...
		expect(prompt).not.toContain("trialSessionAgentId:");
	});
