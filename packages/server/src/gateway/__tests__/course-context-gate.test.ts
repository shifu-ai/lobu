...
describe("course context gate", () => {
	...
	test("retrieves course memory only after a canonical bundle and preserves ready on retrieval failure", async () => {
		const resolved = {
			status: "resolved",
			confidence: "high",
		...
		expect(result).toMatchObject({
			status: "ready",
			context: { retrieval: { status: "failed", eventIds: [], snippets: [] } },
		});
