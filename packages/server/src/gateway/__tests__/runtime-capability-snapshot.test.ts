...
import { fetchRuntimeCapabilitySnapshot, resetRuntimeCapabilitySnapshotCacheForTests, resolveRuntimeCapabilitySnapshot } from "../services/runtime-capability-snapshot.js";
...
describe("runtime capability snapshot transport", () => {
  test("caches by exact identity only until the lesser configured TTL or expiry", async () => {
    ...
    const request = { environment: "production" as const, toolboxUserId: "user-1", agentId: "agent-1" };
    const options = { url: "https://toolbox.test", secret: "secret", fetchImpl, cacheTtlMs: 30_000 };
    await resolveRuntimeCapabilitySnapshot(request, options);
    await resolveRuntimeCapabilitySnapshot(request, options);
    expect(fetchImpl).toHaveBeenCalledTimes(1);
    await resolveRuntimeCapabilitySnapshot({ ...request, agentId: "agent-2" }, {
      ...options,
      fetchImpl: vi.fn(async () => new Response(JSON.stringify(envelope({ agentId: "agent-2" })))),
    });
  });
  test("posts the exact three-field server-only request and accepts a closed digest-bound envelope", async () => {
    ...
    expect(init?.headers).toEqual({ "content-type": "application/json", "x-internal-secret": "server-secret" });
    expect(JSON.parse(String(init?.body))).toEqual({ environment: "production", toolboxUserId: "user-1", agentId: "agent-1" });
  ...
  test.each([
    ...
    await expect(fetchRuntimeCapabilitySnapshot({ environment: "production", toolboxUserId: "user-1", agentId: "agent-1" }, {
      url: "https://toolbox.test/internal/runtime-capabilities",
      secret: "server-secret",
  ...
  test("rejects a bad digest and local as a transport environment", async () => {
    await expect(fetchRuntimeCapabilitySnapshot({ environment: "production", toolboxUserId: "user-1", agentId: "agent-1" }, {
      url: "https://toolbox.test/internal/runtime-capabilities",
      secret: "server-secret",
      fetchImpl: async () => new Response(JSON.stringify({ ...envelope(), snapshotDigest: `sha256:${"0".repeat(64)}` })),
    })).rejects.toThrow(/digest/i);
    await expect(fetchRuntimeCapabilitySnapshot({ environment: "local" as never, toolboxUserId: "user-1", agentId: "agent-1" }, {
      url: "https://toolbox.test/internal/runtime-capabilities", secret: "server-secret",
