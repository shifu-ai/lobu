...
describe("course context tracer", () => {
	it("uses the Toolbox resolver and bundle contract to attach bounded context", async () => {
			...
			.mockResolvedValueOnce(
				new Response(
					JSON.stringify({
						...
						course: {
							courseKey: "super-ai",
							courseEntityId: "course:pm-1:super-ai",
		...
		await attachCourseContextForReviewedScope(payload, {
			baseUrl: "https://toolbox.test/agent-workbench/course-context",
			secret: "secret",
			fetcher,
		});
		expect(payload.resolvedCourseContext).toEqual({
			course: {
				courseKey: "super-ai",
				courseEntityId: "course:pm-1:super-ai",
				displayName: "超級 AI 個體",
	...
	it("enriches the actual MessageConsumer dispatch payload in resolve-arm-send order", async () => {
		...
		const queue = {
			createQueue: async () => {
				order.push(createCount++ === 0 ? "arm" : "create-send");
			},
			send: async (name: string, actual: MessagePayload) => {
		...
		).dispatchCourseContextBoundary(payload, "test-deployment");
	...
	it.each([
				...
				new Response(
					JSON.stringify({
						...
						context: {
							...
							version: 1,
							stale: false,
							confirmedSummary: "x".repeat(8001),
						},
					}),
				),
			],
		...
		await expect(
			attachCourseContextForReviewedScope(payload, {
