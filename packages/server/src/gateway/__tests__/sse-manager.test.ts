...
describe("SseManager cross-replica fan-out", () => {
	test("broadcast forwards the event to the publisher sink", () => {
		const mgr = new SseManager();
		const published: Array<{ agentId: string; event: string; data: unknown }> =
			[];
		mgr.setPublisher((agentId, entry) =>
			published.push({ agentId, event: entry.event, data: entry.data }),
		);

		mgr.broadcast("agent-1", "output", { n: 1 });

		expect(published).toEqual([
			{ agentId: "agent-1", event: "output", data: { n: 1 } },
		]);
	});

	test("setPublisher(undefined) disables fan-out", () => {
		const mgr = new SseManager();
		let calls = 0;
		mgr.setPublisher(() => {
			calls++;
		});
		mgr.setPublisher(undefined);
		mgr.broadcast("agent-1", "output", { n: 1 });
		expect(calls).toBe(0);
	});

	test("deliverFromPeer writes to a local connection without re-publishing", () => {
		const mgr = new SseManager();
		let published = 0;
		mgr.setPublisher(() => {
			published++;
		...
		const stream = fakeStream();
		mgr.addConnection("agent-1", stream);

		mgr.deliverFromPeer("agent-1", {
			event: "output",
			data: { from: "peer" },
			timestamp: Date.now(),
		});
		...
		expect(stream.events[0]?.data).toBe(JSON.stringify({ from: "peer" }));
		// Re-delivering a peer event must NOT bounce it back onto the bus.
		expect(published).toBe(0);
	});

	test("deliverFromPeer drops events for agents with no local connection (no backlog seeded)", () => {
		const mgr = new SseManager();
		...
		// No connection here, so the peer event is dropped — peers must not
		// accumulate backlog for agents they never serve.
		expect(mgr.getRecentEvents("agent-1")).toEqual([]);
	});
});
