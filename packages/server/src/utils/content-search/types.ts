...
export interface ContentSearchOptions {
	...
	agent_id?: string;
	/** Exact external course ids stored in event metadata by context-pack ingestion. */
...
export function isDateFeedMode(options: ContentSearchOptions): boolean {
	return (
		(options.sort_by ?? "date") === "date" &&
		(options.sort_order ?? "desc") === "desc"
	);
}
...
export function resolveDateCursor(
...
export function buildDateCandidateOrderBy(
	cursor: DateCursor | null,
	tableAlias: string,
): string {
...
export function buildPageInfo(params: {
	...
	if (params.useDateFeed) {
		...
		const hasOlder =
			params.cursor?.direction === "after" ? true : fetchedCount > params.limit;
		const hasNewer =
