...
import {
	buildLatestClassificationsCteSql,
	buildThreadMetaCteSql,
} from "./ctes";
import {
...
export async function searchContentBySingleQuery(
	...
	const untilDate = options.until
		? toEndOfDay(parseDateAlias(options.until).date)
		: null;
	const connectionIdsArray =
		options.connection_ids && options.connection_ids.length > 0
	...
	const needClassifications =
		...
		(options.classification_filters &&
			options.classification_filters.length > 0);
	...
	const entityLinkParamIdx =
		visibilityParamIdx + visibilityClause.params.length;
	...
	const vecParam = vectorParamIdx
		? `$${vectorParamIdx}::vector`
		: "NULL::vector";
	...
	const modelScopeFor = (alias: string) =>
		`${alias}.embedding_model = ${configuredModelLiteral}`;
  ...
  `;
	...
	const outerScoreTiebreaker =
		"(f.id % 997) ASC, f.occurred_at DESC, f.id DESC";
	const innerScoreTiebreaker =
		"(fi.id % 997) ASC, fi.occurred_at DESC, fi.id DESC";
	...
	const queryParams: unknown[] = [
		...
		options.semantic_type
			? pgTextArray(
					Array.isArray(options.semantic_type)
						? options.semantic_type
						: [options.semantic_type],
				)
			: null,
	...
	if (useCandidatePath) {
		...
		try {
			rawRows = (await sql.begin(async (tx: DbClient) => {
				await tx.unsafe(
					`SET LOCAL statement_timeout = ${CANDIDATE_QUERY_TIMEOUT_MS}`,
				);
