...
import {
	...
	type AgentConfigurationBootstrapResult,
...
import { AgentReleaseError } from "./agent-release-service.js";
...
const BASELINE_VERSION_PATTERN = /^personal-agent-baseline-v[12]-[0-9a-f]{64}$/;
...
export function createProvisioningRoutes(
	...
	const runtimeCapabilitySnapshotResolver =
		options.runtimeCapabilitySnapshotResolver ??
		resolveRuntimeCapabilitySnapshot;
	...
	provisioningRoutes.post("/agents", async (c) => {
		...
		let provisioned: AgentConfigurationBootstrapResult;
		try {
			const digest = requestDigest({
				agentId,
				name,
	...
	provisioningRoutes.post(
		...
		async (c) => {
			...
			const configuredEnvironment =
				...
				process.env.AGENT_RELEASE_ENVIRONMENT;
			if (
			...
			let snapshot: Awaited<
				ReturnType<typeof resolveRuntimeCapabilitySnapshot>
			>;
			try {
