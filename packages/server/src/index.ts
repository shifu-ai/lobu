...
// Flip an org's provenance between UI- and code-managed. Code-managed orgs are
// owned by a `lobu.config.ts`: `lobu apply` prunes definitions removed from it.
// Same owner/admin gate as visibility — pruning is a destructive capability.
app.patch('/api/:orgSlug/organization/managed-by', mcpAuth, async (c) => {
  const organizationId = c.get('organizationId');
  const memberRole = c.get('memberRole');
  ...
  if (memberRole !== 'owner' && memberRole !== 'admin') {
    return c.json(
      {
        error: 'forbidden',
        message: 'Changing org management mode requires owner or admin access.',
      },
      403
    );
  }
  ...
  if (authSource === 'pat') {
    return c.json(
      {
        error: 'forbidden',
        message: 'Use OAuth or a web session to change org management mode.',
      },
      403
    );
  }
  ...
  if (authSource === 'oauth' && !scopes.includes('mcp:admin')) {
    return c.json(
      {
        error: 'forbidden',
        message: 'Changing org management mode requires mcp:admin scope.',
      },
      403
    );
  }

  let body: { managed_by?: unknown };
  try {
    body = await c.req.json();
  } catch {
    return c.json({ error: 'invalid_request', message: 'Request body must be JSON.' }, 400);
  }
  const managedBy = body.managed_by;
  if (managedBy !== 'ui' && managedBy !== 'code') {
    return c.json(
      { error: 'invalid_request', message: 'managed_by must be "ui" or "code".' },
      400
    );
  }

  const sql = getDb();
  const rows = await sql<{ id: string; slug: string; managed_by: 'ui' | 'code' }>`
    UPDATE "organization"
    SET managed_by = ${managedBy}
    WHERE id = ${organizationId}
    RETURNING id, slug, managed_by
  `;
  const org = rows[0];
  if (!org) {
    return c.json({ error: 'not_found', message: 'Workspace not found.' }, 404);
  ...
  invalidateOrgSlugCache(c.req.param('orgSlug'));
  invalidateOrgSlugCache(org.slug);
  invalidationEmitter.emit(org.id, { keys: ['organizations', 'resolve-path'] });
