...
describe("conversation authorization", () => {
  ...
  // --- Tenant-escape + revocation ---
    ...
    // Agent B replays agent A's handle — must fail closed.
		const escaped = await resolveAuthorizedTarget(
			"agent-b",
			org.id,
			aTarget!.handle,
		);
    ...
    const org = await createTestOrganization();
		...
		expect(
			await resolveAuthorizedTarget(agent.agentId, org.id, "c_not-base64!!"),
		).toBeNull();
		expect(
			await resolveAuthorizedTarget(agent.agentId, org.id, "garbage"),
		).toBeNull();
    // A well-formed handle for a channel the agent is NOT bound to.
		const forged =
			"c_" + Buffer.from("slack:C0OTHER", "utf8").toString("base64url");
		expect(
			await resolveAuthorizedTarget(agent.agentId, org.id, forged),
		).toBeNull();
  });
  ...
  // --- Thread handles ---
    ...
    const [t] = await resolveAddressableTargets(agent.agentId, org.id);
		...
		const thread = await resolveAuthorizedThread(
			agent.agentId,
			org.id,
			threadHandle,
		);
    ...
    // Another agent cannot use that thread handle.
		await createTestAgent({ organizationId: org.id, agentId: "agent-b" });
		expect(
			await resolveAuthorizedThread("agent-b", org.id, threadHandle),
		).toBeNull();
  });
});
