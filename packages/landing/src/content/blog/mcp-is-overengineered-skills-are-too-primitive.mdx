...
4. Upload results via the cloud-storage connector

Never delete source files.
```

`nixPackages` installs ffmpeg, jq, and mediainfo via Nix. `network.allow` configures the sandbox to only allow those two domains. The cloud storage capability is installed as a connector, so it gets connection auth, approval, and audit behavior. The markdown body gets injected into the agent's system prompt. If you want to lock down tool visibility or pre-approve actions, that lives in `lobu.config.ts`, not in `SKILL.md`.

When a user enables this skill, the gateway resolves every section. The worker starts with those binaries on `$PATH`, and the network sandbox blocks everything except the declared domains. The agent starts and has no idea any of this happened.
...
```bash
$ npx @lobu/cli@latest agent config patch my-agent --file skill.patch.json
  Installed "lobu"
  → ./skills/lobu
