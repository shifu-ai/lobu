...
In file-first projects, Lobu resolves persistent memory from `[memory.lobu]` in `lobu.toml`.

- **`[memory.lobu]` enabled**: Lobu derives an Lobu MCP endpoint and uses `@lobu/openclaw-plugin`, falling back to native memory if the plugin is unavailable.
...
1. Gateway resolves an effective Lobu endpoint from `[memory.lobu]` (and optional `MEMORY_URL` override), then selects the default memory plugin and sets it in `agentDefaults.pluginsConfig`.
