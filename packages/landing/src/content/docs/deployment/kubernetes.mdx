...
- Runtime secrets for Lobu (`ENCRYPTION_KEY`, `BETTER_AUTH_SECRET`, provider API keys, OAuth client secrets, and connector credentials as needed).
...
kubectl -n lobu create secret generic lobu-secrets \
  --from-literal=ENCRYPTION_KEY='replace-with-a-long-random-value' \
