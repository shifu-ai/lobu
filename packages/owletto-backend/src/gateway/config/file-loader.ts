...
async function loadAndValidateToml(
  ...
  // Migration check: the `connections` key on agent blocks was renamed to
  // `platforms`. We surface a loud validation error so users update their
  // lobu.toml instead of silently dropping the now-unknown key.
  ...
  if (renamedAgentIds.length > 0) {
    ...
    logger.error("Invalid lobu.toml schema", {
      details: renamedAgentIds.map(
        (id) =>
          `[[agents.${id}.connections]] was renamed to [[agents.${id}.platforms]] — update your lobu.toml`
      ),
    });
    throw new Error(
