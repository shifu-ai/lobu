...
// 右側統一補 (?!性)：擋「後天性」「先天性」這類固定醫學詞後綴（後天性免疫
// 缺陷症候群、先天性與後天性因素分析）。取捨：這也會連帶擋掉「明天性能測試」
// 這種巧合後接「性」的正常句——fail-open 可接受，詳見最終修復報告。
...
const REL_DAY_RE =
  /(?<![目之以大])(前天)(?!性)|(?<![至如現现當当])(今天)(?!性)|(?<![聰聪發发證证查表阐闡說说])(明天)(?!性)|(?<![以之落前大然最午稍])([後后]天)(?!性)|(昨天)(?!性)/g;
...
// 詞後已緊跟日期（含括號包裹）→ 不重複標。三種形狀：ISO（2026-08-10）、
// 中式年月日（2026年8月10日，年可省略）、短斜線/月（8/10、8月10）。
const ALREADY_DATED_RE =
  /^\s*[（(]?\s*(?:\d{4}-\d{1,2}-\d{1,2}|(?:\d{2,4}\s*年\s*)?\d{1,2}\s*月\s*\d{1,2}\s*日?|\d{1,2}\s*[/月]\s*\d{1,2})/;
