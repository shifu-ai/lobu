...
describe("右側複合詞邊界（Codex review #1：後天性/先天性）", () => {
  test("後天性免疫缺陷症候群 不可誤標成 後天（…）性…", () => {
    expect(run("後天性免疫缺陷症候群")).toBe("後天性免疫缺陷症候群");
  });
  test("先天性與後天性因素分析 不可誤標", () => {
    expect(run("先天性與後天性因素分析")).toBe("先天性與後天性因素分析");
  });
});

describe("已有「年月日」格式防重標（Codex review #2）", () => {
  test("昨天（2026年8月10日）的戰報 不可重複標成 昨天（2026-08-10）（2026年8月10日）…", () => {
    expect(run("昨天（2026年8月10日）的戰報")).toBe(
      "昨天（2026年8月10日）的戰報"
    );
  });
});
