...
describe("GatewayClient heartbeat ACKs", () => {
  ...
  test('normalizes an exact trusted pre-Task2 retrieval and sends its delivery receipt',async()=>{const requests:unknown[]=[];globalThis.fetch=mock(async(_url,init)=>{requests.push(JSON.parse(String(init?.body)));return new Response('ok');}) as typeof fetch;const client=new GatewayClient('https://gateway.example.com','worker-token','user-1','worker-1');const handleThreadMessage=mock(async()=>undefined);(client as any).handleThreadMessage=handleThreadMessage;const context:any=validResolvedCourseContext();context.retrieval.snippets=[{eventId:5,title:'A',text:'A only',sourceUrl:null}];await (client as any).handleEvent('job',JSON.stringify({jobId:'legacy-job',payload:{...basePayload(),resolvedCourseContext:context}}));expect(requests).toContainEqual({jobId:'legacy-job',received:true});expect(handleThreadMessage).toHaveBeenCalledTimes(1);expect(handleThreadMessage.mock.calls[0]?.[0].resolvedCourseContext.retrieval.snippets).toEqual([{eventId:5,title:'A',text:'A only',sourceUrl:null,sourceRef:'lobu:event:5',provenanceKind:'fresh_course_retrieval',courseEntityId:'course:user:course-a',readinessFields:{}}]);});
