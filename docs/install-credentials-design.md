...
| First-device web/menubar UX | Sign-up with email + passkey enrolment | "Paste install secret" once → enrol passkey, never see it again (1Password Secret Key pattern) — same flow on every subsequent new device |
...
## Device pairing: paste-once, then passkey

**The user-visible model is uniform across all devices and clients:
paste `ENCRYPTION_KEY` once on a new device, then enrol a passkey
(plus optional macOS passcode) for everyday unlock.** This matches
the "Web / menubar — first-open flow" above and applies the same
way to second / third / Nth devices.

No `pairing.url` file. No `single_use` PAT. No `POST /auth/pair-token`
endpoint. No `/auth/enrol-credential` SPA route as a redirect target
for a pairing URL. Better-auth's existing passkey plugin
(`packages/server/src/auth/index.tsx:536`) plus the existing
passkey enrolment / sign-in surface in the SPA login
(`packages/owletto/src/app/auth/login.tsx:640`) already cover what
the GUI clients need. For second-device handoff specifically,
browser-native **WebAuthn cross-device verification** (caBLE /
hybrid transport — phone scans a QR generated by the browser,
approves sign-in on the laptop) is implemented by the platform,
not by us, and addresses the menubar/extension second-device case
without any new server-side infrastructure.

### Flow

```
   lobu init                            → writes .env with ENCRYPTION_KEY  (today, unchanged)
   lobu run (first boot)                → ensureInstallOperator() provisions
                                          install_operator user with
                                          better-auth-hash(ENCRYPTION_KEY)
                                          as the credential
                                                       │
                                                       ▼
   any GUI client (menubar / extension / SPA) on any device:

      first open  →  "Connect this device" screen (paste ENCRYPTION_KEY)
                     POST /api/auth/sign-in/email
                     Set-Cookie session
                     route to passkey enrolment (existing flow)

      subsequent  →  passkey unlock (existing flow)
```

The CLI on the install host reads `.env` directly and
POSTs to `/api/auth/sign-in/email` — no GUI paste step needed,
because the secret is already on disk where the CLI runs.

### What stays the same (vs. today)

- `ENCRYPTION_KEY` is the install operator's better-auth password
  (v3 core).
- `ENCRYPTION_KEY` encrypts at-rest data (today's behaviour).
- CLI reads `.env` directly; no pairing artefact for CLI.
- `PersonalAccessTokenService` shape unchanged — no `single_use`
  column.
- Existing passkey enrolment / sign-in screens in
  `packages/owletto/src/app/auth/login.tsx` are reused as-is. After
  install-secret sign-in, the SPA routes the user to the passkey
  settings page; no new "enrol-credential" route is added.

### What changes

- `ensureInstallOperator()` boot hook provisions the install
  operator row. Idempotent. (No pairing-file side effect.)
- "Connect this device" screen on first open against an install
  with `hasInstallOperator=true && currentDeviceHasSession=false`.
- macOS menubar adds an optional passcode unlock over a stored PAT
  (addendum #1).

### `install_operator` carve-outs

The install operator is a `user` row, but it is **not a human
identity**. It must be excluded from every flow that assumes
"user = signed-up human." Concretely, `principal_kind =
'install_operator'` is a filter on:

- **Password reset / forgot-password** flows (better-auth's
  built-in `/forget-password` + `/reset-password`). The install
  operator can't reset because its credential is a hash of
  `ENCRYPTION_KEY`, not a human-managed password — re-deriving it
  on the server would require knowing `ENCRYPTION_KEY` plaintext
  again. Server returns `404` for the synthetic operator email.
- **Magic-link login** (if/when wired). Install operator emails
  resolve to `404` for magic-link requests.
- **OAuth account-linking** (Google/GitHub/etc. → existing
  account). The install operator can't be the target of an OAuth
  link — would let any signed-in human upgrade themselves into the
  system actor.
- **Admin UI member lists** (org members page in the SPA). The
  install operator never appears in the org's member roster even
  though it's the org owner row at the DB level.
- **Org-member UI more broadly** — invitations, role-edit pages,
  "members" filters, all skip the install operator.
- **Account-discovery / user-enumeration endpoints** (anything
  that returns "is there a user with email X?"). Synthetic
  operator emails always answer "no."
- **Identity-graph / subject-identities tables** (where humans
  get profile rows used for recall / event attribution). The
  install operator does not produce subject-identity rows; events
  authored by it render with name "Install operator" and don't
  participate in identity-graph stitching.
- **Single-user-mode count** (already noted in "Multi-tenant
  mode" — the count predicate is
  `WHERE principal_kind <> 'install_operator'`).

The carve-outs are enforced as **WHERE clauses on the
appropriate queries**, not as a separate code path. The default
shape is "every user query filters out
`principal_kind='install_operator'` unless the caller is the
install-operator-aware path (`ensureInstallOperator`, the
`/api/local-init` session-to-PAT exchange, key-rotation
tooling)."

### Password-manager UX

The install-secret paste screen looks like a normal password
input. The existing login UI calls `navigator.credentials.store(...)`
(`packages/owletto/src/app/auth/login.tsx:48,292`) after a
successful sign-in so password managers (1Password, Keychain,
browser-native) auto-offer to save the typed value. **For the
install secret this is the intended behaviour.** The user
benefits from password-manager autofill on subsequent new devices
(the paste step is the slowest part of pairing), and the secret is
already at the same trust tier the password manager protects.

So: **explicitly opt-in to `credentials.store` on the install-secret
screen**, the same way the existing login form does. Don't set
`autocomplete="off"`, don't use the `prefer-no-credentials` API.
Document the choice next to the screen's React component so a
future reviewer doesn't "fix" it.

Operators who don't want password-manager storage can decline the
manager's prompt at paste time (browser-native UX) or strip the
saved entry afterwards. Imposing it server-side would optimise for
a minority case and lose autofill for the majority.

---

## Future: alternative device pairing transports

The Stage 2 design keeps device pairing minimal: paste-once + passkey.
This works wherever the user can copy `ENCRYPTION_KEY` from a
password manager / Keychain / wherever they store it.

WebAuthn cross-device verification likely covers the menubar /
Chrome-extension second-device case natively. If real-world usage
shows transports it doesn't cover (e.g. CLI-managed extension
installs on hosts without WebAuthn, headless second-host
provisioning where the user can't paste interactively), the
fallback design below is what we'd build. **Do not build it until
it's proven necessary.**

### Fallback design — reuse `/exchange-token` + single-use PATs

`/exchange-token` (`packages/server/src/auth/routes.ts:292`) already
takes a token, resolves it through `resolveDeepLinkToken`, mints a
session cookie, and redirects to a relative `next` path with
`Referrer-Policy: no-referrer`. `PersonalAccessTokenService` already
covers create/verify/revoke. The minimum delta to use this for
pairing would be:

- **One column** on `personal_access_tokens`: `single_use boolean
  NOT NULL DEFAULT false` + partial index on
  `(single_use) WHERE single_use=true AND revoked_at IS NULL`.
- **Auto-revoke on verify** in `PersonalAccessTokenService.verify()`
  when `single_use=true`. See the race-condition warning below.
- **`ensureInstallOperator()` boot path** writes
  `~/.lobu/install/<install-id>/pairing.url` (mode `0600`) at first
  boot, idempotent.
- **`POST /auth/pair-token`** (authenticated) to mint a fresh
  single-use PAT for the caller's own user, returned as a URL.
- **`/auth/enrol-credential`** SPA route as the `next=` redirect
  target after `/exchange-token` consumes the pairing PAT.

### Race-condition warning for any future single-use PAT design

A naive SELECT-then-UPDATE pattern in
`PersonalAccessTokenService.verify()` lets two concurrent requests
both pass: both reads see `revoked_at IS NULL`, both proceed, then
both updates run and the second one is a harmless no-op — but both
callers already received a valid verdict and got session cookies.
Any single-use implementation must use:
...
UPDATE personal_access_tokens
 ...
 RETURNING id, user_id, scope, name, expires_at;
```

Zero affected rows ⇒ verification failure (revoked or never
existed). Only the row that this `UPDATE` actually mutated counts
as "verified." Don't ship a `SELECT … WHERE revoked_at IS NULL`
...
2. **`lobu init` doesn't prompt** — the secret is still generated
   ...
   `WHERE principal_kind <> 'install_operator'`.
3. **GUI clients paste `ENCRYPTION_KEY` once per new device.**
   On first open against an install with an install operator but
   no local session, the SPA / menubar / extension shows a
   "Connect this device" screen that POSTs the pasted secret to
   `/api/auth/sign-in/email`, then routes to the existing
   passkey-enrolment surface. Every subsequent open uses the
   passkey (or macOS passcode unlock over a stored PAT). No
   pairing URL file, no single-use PAT machinery, no new
   `/auth/pair-token` endpoint, no new SPA route — Stage 2 reuses
   better-auth's existing passkey plugin and the SPA's existing
   passkey enrolment flow. The browser-native WebAuthn cross-device
   verification path (caBLE / hybrid) handles second-device sign-in
   without server-side help. CLI on the install host reads `.env`
   directly.
...
3. **"Connect this device" SPA screen**: new route shown when
   `hasInstallOperator=true && currentDeviceHasSession=false` —
   single password-style input, submit calls
   `/api/auth/sign-in/email` with the synthetic operator email +
   pasted value, then routes to the existing passkey-enrolment
   page on success. Allow `navigator.credentials.store` so password
   managers can offer to save the secret (intended UX — see
   "Password-manager UX"). Inline form, no modal
   (DESIGN_GUIDELINES §1 + §8).
4. **`/api/auth/config` returns `hasInstallOperator`** so the SPA
   knows whether to show the "Connect this device" screen vs
   `/sign-up` (for multi-tenant cloud installs) vs the normal sign-in
   page.
5. **Install-operator carve-outs**: filter
   `principal_kind='install_operator'` out of password-reset,
   forgot-password, magic-link, OAuth account-linking, admin
   member-list, org-member, account-discovery, and
   identity-graph / subject-identities queries. Plus the
   single-user-mode count in `auth/index.tsx:567` and the
   `hasUser` query in `auth/config.ts`. Update the
   `COMMENT ON TABLE public."user"` text in the same PR.
...
8. **macOS menubar passcode unlock** (addendum #1, unchanged) — a
   4–6 digit PIN gates a stored PAT in macOS Keychain via
   `kSecAttrAccessControlUserPresence`. Independent of the
   install-secret paste step.
9. **E2E tests**: (a) `lobu init` → `lobu run` → `lobu login`
   works; (b) SPA "Connect this device" → paste `ENCRYPTION_KEY`
   → passkey enrolment works on a fresh browser profile;
   (c) restart `lobu run` + `lobu login` still works (operator
   persists); (d) wrong `ENCRYPTION_KEY` → 401;
   (e) `/api/local-init` rejects unauthenticated callers;
   (f) password-reset for the synthetic operator email returns
   404; (g) admin member-list excludes the install operator row.
