...
-- =============================================================================
-- Lobu schema baseline (2026-05-19)
-- =============================================================================
--
-- Single source of truth for the schema. Replaces 82 prior migration files
-- (the old 00000000000000_baseline.sql + 81 forward deltas).
--
-- Generated by:
--   1. Apply all 82 historical migrations to a fresh pgvector/pgvector:pg16 DB
--   2. Drop dead schema flagged by audit:
...
COMMENT ON TABLE public.agents IS 'Top-level agent definitions per organization. Each row is one configured AI agent: model selection, system prompt (soulMd/identityMd/userMd), MCP tool surface, network/nix/plugins config, secret refs, and tool-approval policy.';
...
--
-- Name: TABLE auth_profiles; Type: COMMENT; Schema: public; Owner: -
--
...
COMMENT ON TABLE public.connections IS 'Per-organization credentialed connections to external platforms / connectors (Slack, Telegram, GitHub, Linear, …). Stores platform-specific config + secret:// refs. Replaces the legacy chat_connections + per-platform tables.';
...
COMMENT ON TABLE public.events IS 'Append-only event log — the primary data substrate. Each row is a discrete observation from a connector or user action. Watchers, semantic search, and entity extraction all derive from here. Never DELETE; supersede with a tombstone row that points at the original via supersedes_event_id.';
...
COMMENT ON TABLE public.entities IS 'Domain objects tracked per organization (people, companies, tasks, …). Type definition lives in entity_types; the row body is jsonb keyed by the type''s columns.';
...
--
-- Name: TABLE entity_types; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.entity_types IS 'Per-organization type definitions for the entities table — column schema, defaults, classifiers, and the lifecycle hooks each type triggers.';
...
--
-- Name: TABLE event_classifications; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.event_classifications IS 'Per-event labels produced by classifiers (embedding / llm / user-driven). Multiple classifications per event; the latest per (event, type) is exposed via the latest_event_classifications view.';
...
COMMENT ON TABLE public.feeds IS 'Per-organization content feeds derived from a connection (e.g., Slack channel → feed, RSS source → feed). Watchers and agents subscribe to feeds.';
...
COMMENT ON TABLE public.member IS 'Junction between user and organization with a role (owner / admin / member). The auth source of truth for permission checks at the org level.';
...
COMMENT ON TABLE public.oauth_tokens IS 'OAuth 2.1 access + refresh tokens issued by the gateway. Covers MCP clients (Claude Desktop, Cursor, etc.) and integration consumers. Tied to oauth_clients (the registered app) and the user via subject.';
...
COMMENT ON TABLE public.organization IS 'Tenant root. Every other table scopes to an organization. The slug is the public identifier; id is the immutable key used in joins.';
...
COMMENT ON TABLE public.personal_access_tokens IS 'PATs for CLI, MCP, and worker bridges. Optional worker_id binding (non-NULL) restricts the token to one device_workers.worker_id, used by child tokens minted via /api/me/devices/mint-child-token. Plain user PATs leave worker_id NULL.';
...
COMMENT ON TABLE public.runs IS 'Job queue for the gateway. Each row is a single sync / action / embed_backfill / auth / watcher / chat_message / schedule / agent_run / internal / task invocation. status transitions pending → claimed → running → completed | failed | timeout via heartbeat-based reaping.';
...
--
-- Name: TABLE watchers; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON TABLE public.watchers IS 'Per-agent scheduled jobs with a cron-style cadence. Each tick enqueues a runs row of run_type=''watcher''. The runs row is the unit of execution; this table is the schedule definition.';
